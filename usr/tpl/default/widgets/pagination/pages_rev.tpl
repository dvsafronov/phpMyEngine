...
list($to, $from) = $calcLine($paginationCurPage, $o, $paginationCount);
            ...
            <div class="pagination">
                <ul>
                    <?php for ($i = $from; $i != $to; $i--) : ?>
                        <?php if ($i != $paginationCurPage): ?>
                            <li>
                                <a href="<?php echo $paginationHREF ?>/page<?php echo($i) ?>"><?php echo($i) ?></a>
                            </li>
                        <?php else: ?>
                            <li class="active disabled"><span><?php echo($i) ?></span></li>
                        <?php endif ?>
                    <?php endfor ?>
                </ul>
            </div>
            <div class="pager">
                <ul>
                    <?php if ($paginationCount > 1 and $paginationCurPage != $paginationCount) : ?>
                        <li><a class="fal"
                               href="<?php echo $paginationHREF ?>/page<?php echo($paginationCount) ?>">Последняя</a>
                        </li>
                    <?php else: ?>
